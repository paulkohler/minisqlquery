...
{
	/// <summary>An interface for simple plugins for use in extending Mini SQL Query.
	/// Plugins are loaded from DLL's in the working directory matching the pattern "*.PlugIn.dll"</summary>
	/// <remarks><para>Plugins are created during the load process.
	...
	public interface IPlugIn
	{
		...
		/// <summary>Plugin load order. For external plugins start with values over 1000. 
		/// This is a simple way of handling dependencies of other services etc.</summary>
		/// <value>The requested load order.</value>
		int RequestedLoadOrder { get; }

		/// <summary>Initializes the plug in, called after the main form is displayed.</summary>
		void InitializePlugIn();

		/// <summary>Loads the plugin and stores a reference to the service container.
		/// Called at application startup time.</summary>
		...
		/// <summary>Called when the plugin is unloading (typically application shutdown).</summary>
		void UnloadPlugIn();
	}
}